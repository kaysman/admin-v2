...
class _UploadExcelDialogState extends State<UploadExcelDialog>
  ...
  Widget build(BuildContext context) {
    isTenant = AppService.currentUser.value?.role?.name == "TENANT";
    return SizedBox(
      width: 402.w,
                          ...
                          ...[
                            ...
                            if (isTenant) ...[
                              BlocBuilder<MerchantBloc, MerchantState>(
                                builder: (context, state) {
                                  if (state.listMerchantStatus ==
                                      ListMerchantStatus.loading)
                                    return const Center(
                                        child: CircularProgressIndicator());
                                  else if (state.listMerchantStatus ==
                                      ListMerchantStatus.error) {
                                    return Text(
