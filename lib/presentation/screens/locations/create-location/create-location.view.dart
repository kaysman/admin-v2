...
class _CreateLocationState extends State<CreateLocation>
  ...
  WarehouseType? selectedType = WarehouseType.WAREHOUSE;
  onTypeChanged(WarehouseType? v) {
    setState(() {
      selectedType = v;
    });
  }
  ...
  void checkIfLocationIsUpdating() {
    if (LocationService.selectedLocation.value != null) {
      ...
      _locationTypeController.text = location?.type?.name ?? '';
  ...
  submitForm(
    ...
    Address _address = Address(
      ...
      specificTypeOfLocation: SpecificTypeOfLocation.NOT_APPLICABLE,
      typeOfContactForAddress: TypeOfContactForAddress.WAREHOUSE,
    );

    ContactDetail contactDetail = ContactDetail(
      firstName: _contactNameController.text,
