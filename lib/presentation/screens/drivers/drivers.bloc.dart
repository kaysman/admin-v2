...
class DriverBloc extends Cubit<DriverState> {
  ...
  updateGlobalSelectedDriver(Map<String, dynamic> data) {
    print(data);
    User _updatedUser = User(
      ...
      driver: DriverDetails.fromJson(data['driver']),
    );
  ...
  saveDriver(
    ...
    emit(state.updateDriverState(
        createDriverStatus: CreateDriverStatus.loading));
    try {
      var result = await UserService.createDriver(data, isUpdating);
      if (result.success == true) {
        if (isUpdating) {
          updateGlobalSelectedDriver(result.data);
        }
        showWhiteDialog(
          context,
          ResponseDialog(
...
class DriverState {
  ...
  final User? driverBeingViewed;

  DriverState({
    this.listDriverStatus = ListDriverStatus.loading,
    this.drivers,
    this.driverBeingViewed,
    this.driverItems,
    this.perPage = 10,
